...
export default function Dashboard() {
  ...
  const ticketPromedio = ventasFiltradas.length > 0 ? totalPeriodo / ventasFiltradas.length : 0;
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4">
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            {/* tabs */}
            <div className="flex gap-1 bg-gray-100 dark:bg-gray-700/60 rounded-xl p-1 w-fit">
            ...
            <div className="flex items-center gap-2">
              ...
              {tipo === 'dia' && (
                <input type="date" value={diaVal} max={hoy.toISOString().slice(0, 10)}
                  ...
                  className="text-sm px-3 py-1.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              )}
              {tipo === 'mes' && (
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500 hidden sm:block ml-auto capitalize font-medium">{labelPeriodo}</span>
          </div>
        </div>
        ...
        {puedeVerVentas && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            />
            <StatCard icono={ShoppingBag} titulo="Ticket promedio" valor={fmt(ticketPromedio)}
              sub="Por pedido cobrado" color="emerald" cargando={cvVentas} delay={160}
            />
            <StatCard
              icono={Wallet}
            ...
            {cvVentas ? (
              ...
              <div style={{ height: 250 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={datosArea} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <defs>
                    ...
                    <XAxis
                      ...
                      interval={tipo === 'dia' ? 1 : tipo === 'mes' ? 3 : 0}
                    />
                    <YAxis
                      yAxisId="left"
                      tick={{ fontSize: 11, fill: tickColor }}
                    ...
                    <Legend
                      ...
                      formatter={v => <span style={{ fontSize: 12, color: tickColor }}>{v === 'total' ? 'Ingresos (Bs)' : 'Pedidos'}</span>}
                    />
                    ...
                    />
                    <Area yAxisId="right" type="monotone" dataKey="pedidos" name="pedidos"
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={datosPedidos} margin={{ top: 4, right: 4, left: 0, bottom: 0 }} barGap={3} barCategoryGap="30%">
                  ...
                  <Legend
                    iconType="circle" iconSize={8}
                    formatter={v => (
                      <span style={{ fontSize: 12, color: tickColor }}>
