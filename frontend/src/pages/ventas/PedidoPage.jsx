...
export default function PedidoPage() {
  ...
  const { data: productos = [], isLoading: cargandoProductos } = useQuery({
    ...
    queryFn: () => getProductos({ solo_vendibles: true, ...(categoriaActiva ? { categoria_id: categoriaActiva } : {}) }),
  });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="flex flex-1 gap-4 overflow-hidden pt-4">
        ...
        {/* ── Panel derecho: orden ────────────────────────────────── */}
        <aside className={`flex flex-col w-full md:w-80 lg:w-96 shrink-0 overflow-hidden ${tabMobile === 'productos' ? 'hidden md:flex' : 'flex'}`}>
          <div className="flex flex-col flex-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
            {/* Cabecera panel */}
            <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2 shrink-0">
              <ShoppingCart className="w-4 h-4 text-gray-500 dark:text-gray-400" />
              <span className="font-semibold text-sm text-gray-700 dark:text-gray-200">
                Orden — {pedido.detalles?.length ?? 0} ítem(s)
              </span>
            </div>
                ...
                <div className="space-y-2">
                  {puedeCobrar && (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl font-semibold text-sm transition-colors"
                    >
...
function ModalCobrar({ total, pedidoId, usuarioId, onClose, onExito }) {
  ...
  const montoInsuficiente = metodo === 'efectivo' && montoRecibido && parseFloat(montoRecibido) < total;
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <button
                key={m.id}
                onClick={() => { setMetodo(m.id); setMontoRecibido(''); setError(null); }}
        ...
        {metodo === 'efectivo' && (
          <div>
            ...
            <input
              ...
              autoFocus
              value={montoRecibido}
              onChange={e => { setMontoRecibido(e.target.value); setError(null); }}
              placeholder={total.toFixed(2)}
              className={`w-full bg-white dark:bg-gray-700 border rounded-xl px-4 py-2.5 text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 transition ${
                montoInsuficiente
            ...
            />
            {montoInsuficiente && (
              <p className="text-xs text-red-500 mt-1">El monto es menor al total</p>
            )}

            {/* Cambio */}
            {montoRecibido && !montoInsuficiente && (
              <div className="flex items-center justify-between mt-3 bg-green-50 dark:bg-green-900/20 rounded-xl px-4 py-3">
                <span className="text-sm text-green-700 dark:text-green-400 font-medium">Cambio</span>
        ...
        <div className="flex justify-end gap-3 pt-1">
          ...
          <button
            ...
            disabled={cobrar.isPending || (metodo === 'efectivo' && (!montoRecibido || montoInsuficiente))}
